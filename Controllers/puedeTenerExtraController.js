...
exports.porArt = async (req, res) => {
  const idArt = req.query.idArt;
  const extras = await prisma.puedeTenerExtra.findMany({
    where: {
      idArt: Number(idArt),
    },
    select:{
  ...
  //   pool
  //     .query(
  //       `select pte.idE, Nombre, Descripcion,Precio from PuedeTenerExtra pte, Extra e where pte.idE = e.idE and idArt = ${idArt};`
  //     )
  //     .then((response, err) => {
  //       if (err) {
  //         console.log(err);
...
exports.quitarPTE = (req, res) => {
  ...
  try {
    pool
      .query(
        `delete from PuedeTenerExtra where idArt = ${idArt} and idE = ${idE}`
      )
      .then((response, err) => {
        if (err) {
          console.log(err);
